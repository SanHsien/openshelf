...
2. `openshelf export` — 下載可匯出的書：無 DRM 抓 EPUB/PDF（某格式失敗自動改試另一種），DRM 抓 `.acsm`，無法匯出的只記錄；跳過已下載者。常用旗標：`--refresh-acsm`（重抓逾時的 `.acsm`，以下載時間為準）、`--force-refresh-acsm`（ADE 顯示 E_ADEPT_REQUEST_EXPIRED 時，不看天數強制重抓，建議搭配 `--limit 25` 分批）、`--skip-failed`、`--only`、`--limit`。
3. `openshelf status` / `openshelf report` — 回報統計，並在 `output/下載報表.txt` 產出人可讀報表，重點列出**缺漏的書**（失敗、無法匯出）。
4. `openshelf acsm-open` / `openshelf acsm-report` — 批次用系統預設程式開啟已下載的 `.acsm`（`--limit` 分批、`--include-opened` 重送）；不解析、不轉換。
...
桌面介面：`openshelf ui`（需 `pip install -e '.[gui]'`）。懷疑 Google 後端改版時先跑 `openshelf doctor` 做端點健康檢查。
