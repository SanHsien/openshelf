...
_EN: dict[str, str] = {
    ...
    "請在開啟的瀏覽器完成 Google 登入並看到自己的書庫後，按「OK」繼續。":
        "Sign in to Google in the opened browser; once you can see your "
        "library, press OK to continue.",
    # 首次啟動導覽
...
def detect_default(locale_name: str | None = None) -> str:
    """以語系猜預設語言：zh*／Chinese* → zh，其餘 → en。

    locale_name 由呼叫端提供最可靠（GUI 傳 QLocale.system().name()，如 "zh_TW"）。
    未提供時退回標準庫：getlocale() 在 Windows 可能回 "Chinese (Traditional)_Taiwan"
    這種格式，故同時比對 "chinese"。不用已棄用（3.15 將移除）的 getdefaultlocale()。
    """
    code = (locale_name or "").lower()
    if not code:
        try:
            code = (locale.getlocale()[0] or "").lower()
        except Exception:  # noqa: BLE001 — 取不到語系就再退一層
            code = ""
    if not code:
        code = (os.environ.get("LC_ALL") or os.environ.get("LANG") or "").lower()
