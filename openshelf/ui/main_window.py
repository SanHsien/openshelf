...
class MainWindow(QMainWindow):
    ...
    def on_need_confirm(self, _message: str) -> None:
        QMessageBox.information(
            self,
            tr("完成登入"),
            tr("請在開啟的瀏覽器完成 Google 登入並看到自己的書庫後，按「OK」繼續。"),
        )
...
def run(config: Config) -> int:
    ...
    # QLocale.system().name()（如 "zh_TW"）跨平台最可靠；stdlib 僅作後援
    set_language(
        settings.value("language") or detect_default(QLocale.system().name())
    )
